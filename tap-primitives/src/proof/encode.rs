...
use crate::asset::{Asset, EncodeType, Genesis, GroupKeyReveal};
use crate::commitment::CommitmentProof;
use crate::encoding::asset::{encode_asset, encode_genesis};
...
use crate::proof::types::{AnchorTx, BlockHeader, Proof, TaprootProof, TransitionVersion};
...
/// TLV types for TaprootProof sub-records.
mod taproot_tlv {
...
/// TLV types for CommitmentProof sub-records (matches Go's commitment proof).
mod commitment_tlv {
...
pub fn encode_proof(proof: &Proof) -> Vec<u8> {
    ...
    // Type 27: AltLeaves (optional).
    // Alt leaves in the proof types.rs are stored as part of unknown_odd_types
    // or as a separate field. For now, encode unknown odd types.
...
pub fn encode_taproot_proof(tp: &TaprootProof) -> Vec<u8> {
    ...
    // Type 5: TapscriptProof (optional, for exclusion proofs on bare outputs).
    // Not part of TaprootProof struct -- handled via unknown_odd_types.
...
pub fn encode_commitment_proof(cp: &CommitmentProof) -> Vec<u8> {
    ...
    if let Some(ref ap) = cp.asset_proof {
        ...
        inner.push(TlvRecord::u8(0, ap.version as u8));
...
pub fn encode_group_key_reveal(gkr: &GroupKeyReveal) -> Vec<u8> {
    match gkr {
        ...
        GroupKeyReveal::V1(v1) => {
            // V1: TLV-encoded with version, internal_key, tapscript details.
            let mut stream = TlvStream::new();
...
///
/// Format: `BigSize(count) [32B hash]... [packed_bits]`
fn encode_tx_merkle_proof(proof: &TxMerkleProof) -> Vec<u8> {
    let mut buf = Vec::new();
